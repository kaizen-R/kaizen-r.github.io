...
In 10-bits, **the SAME not-bit-4 classifier works with... 1024 states (total)**. For a perfect data mining exercise, you need to find a ruleset that perfectly matches all 1024 states, and you need to try combinations of **3 possible values per-bit** (our ternary alphabet: {'\#', '0', '1'}). If I'm correct, that's about **59048 possible unique classifiers** (the '\##########' option is not valid for us...). **And** an LCS is expected to output a **combination subset** of these 59K classifiers as an output System, which could be a very large search space indeed...
