...
I have played a lot with demo datasets for data mining, with the Iris dataset, and also more recently with the "monk problems" and "mushrooms" (from UC Irvine). All classics for training Supervised Learning models.
...
My "Rosetta Stone" encoding function can go as high as 6 bits per variable, meaning I can encode up to 64 "bins" for a given variable. But if there are in fact 64 or more unique values, then having more bins means **adding granularity** and bits to the bit string (aka "state", the basic input alongside the "class", for the RLCS training).
...
\*There is a second aspect to this: A future version of my Rosetta Stone encoder might be a little more clever: Instead of using "simple" medians, maybe I can use entropy? If I have three categories, but "one is more important" than the other two, I could then use two bits, one of which might suffice to encode information for the more important category/value. So RLCS could single out that one bit for classification choices. This is inspired by information theory and data compression schemes, really. So it makes perfect sense from a theoretical standpoint. Though in practice, I am unsure how much of an impact this might have... I guess we'll have to wait and see! To the "to-do" list then.
